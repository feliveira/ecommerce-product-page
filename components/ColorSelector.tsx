...
export default function ColorSelector({
  ...
  return (
    <div className="mt-6 flex flex-col">
      <p className="text-lg text-zinc-900">Cor: <span className="font-bold">{selectedColor.name}</span></p>
        ...
        {colors.map((color: ProductColorVariant) => (
          <button
            ...
            className={`relative -m-0.5 flex items-center justify-center rounded-full p-0.5 focus:outline-none transition-all duration-150 ease-in-out
                          ${selectedColor.value === color.value ? 'ring-2 ring-offset-1 ring-zinc-900' : 'hover:ring-1 hover:ring-zinc-400'}`}
            title={color.name}
