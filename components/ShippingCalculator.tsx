...
export default function ShippingCalculator({
  ...
  return (
    <div className="mt-10">
      <h3 className="text-lg font-medium text-zinc-900">Calcular Frete e Prazo</h3>
      ...
      {shippingInfo && (
        <div className="mt-4 p-4 border border-zinc-200 rounded-md bg-zinc-50">
          <p className="text-sm font-medium text-zinc-900">Endereço de Entrega:</p>
          <p className="text-sm text-zinc-700">{shippingInfo.logradouro}, {shippingInfo.bairro}</p>
          <p className="text-sm text-zinc-700">{shippingInfo.localidade} - {shippingInfo.uf}</p>
          <p className="text-sm text-zinc-700">CEP: {shippingInfo.cep}</p>
          <p className="mt-2 text-sm text-green-600">Frete disponível para este endereço.</p>
        </div>
      )}
    </div>
  )
